...
proper non-singleton subset $P\subset Q$, there exists a word $u\in A^*$ of
length at most $\alpha|Q|$ such that $|Pu^{-1}|>|P|$.
...
and thus, we see that the \v{C}ern\'{y} conjecture holds true for such
automata.
...
permits!!}} we include here a proof from~\cite{Kari:2003} as an example of
these linearization techniques.

Given a DFA $\mathcal{A}=(Q,A)$ with $Q=\{1,2,\dots,n\}$, we assign to each
subset $P\subseteq Q$ its \emph{characteristic vector}\index{characteristic
vector} $[P]$ in the  linear space $\mathbb{R}^n$ of $n$-dimensional row
vectors over $\mathbb{R}$ as follows: $i$-th entry of $[P]$ is 1 if $i\in P$,
otherwise it is equal to 0. (For instance, $[Q]$ is the all ones row vector and
the vectors $[1],\dots,[n]$ form the usual orthonormal basis of
$\mathbb{R}^n$.) Further, we assign to each word $w\in A^*$ the linear operator
$\varphi_w$ on $\mathbb{R}^n$ defined by $\varphi_w([i])=[iw^{-1}]$ for each
$i\in Q$.

Now suppose that the digraph $D(\mathcal{A})$ is Eulerian. This means that for
every vertex, its in-degree and its out-degree are equal. In the underlying
graph of a DFA the out-degree of every vertex is equal to the cardinality of
the input alphabet. Hence, if $|A|=k$, then each vertex in 
