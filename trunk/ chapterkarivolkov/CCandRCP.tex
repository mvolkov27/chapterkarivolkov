...
If $|Q|=n$, then the main loop of Algorithm~\ref{KV:Extension} is
executed at most $n-2$ times. Suppose that the input automaton
$\mathcal{A}=(Q,A)$ has the following $\alpha$-\emph{extension
property}\index{$\alpha$-extension property}: for each proper
non-singleton subset $P\subset Q$, there exists a word $u\in A^*$
of length at most $\alpha n$ such that $|Pu^{-1}|>|P|$. If
Algorithm~\ref{KV:Extension} is executed on such an automaton,
then each of at most $n-2$ words $v$ that the algorithm adds in
its main loop has length at most $\alpha n$, whence the length 
