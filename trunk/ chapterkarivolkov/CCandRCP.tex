...
synchronizing and its underlying digraph is strongly connected (since it is
Eulerian), there exists a word $w\in A^*$ such that $Q\cdot w\subseteq S$---one
can first synchronize $\mathcal{A}$ to a state $q$ and then move $q$ into $S$
by applying a word that labels a path from $q$ to a state in $S$. Then
\begin{displaymath}
...
Clearly, if $U_{j+1}=U_j$ for some $\ell$ then $\varphi_a(U_j)\subseteq U_j$
for all $a\in A$ whence $U_i=U_j$ for every $i\ge j$. Let $\ell$ be the least
number such that $\varphi_u(x)\notin U$ for some word $u$ of length $\ell$,
that is, the smallest $\ell$ such that $U\ell\nsubseteq U$. Then in the chain
...
discovered no $n$-state \san\ with Eulerian underlying digraph and \rt\ that
would exceed \dots
