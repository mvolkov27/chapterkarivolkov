...
length at most $\alpha|Q|$ such that $|Su^{-1}|>|S|$.

Suppose that Algorithm~\ref{KV:Extension} is executed on an $\alpha$-extensible
automaton with $n$ states. Then the main loop is executed at most $n-2$ times
and each word that it appends to the current word has length at most $\alpha
n$. Hence the length of the \sw\ returned by the algorithm does not exceed
$1+\alpha n(n-2)$. Observe that this bound is quadratic in $n$ while the
bound~\eqref{KV:eq:pin} that we so far have in general is cubic. Moreover, if
an automaton is 1-extensible, then its \rt\ does not exceed $1+n(n-2)=(n-1)^2$,
and thus, the \v{C}ern\'{y} conjecture holds true for such automata.
...
1-extensibility of \sa\ whose underlying digraphs\footnote{The \emph{underlying
digraph}\index{underlying digraph of an automaton} $D(\mathcal{A})$ of a DFA
$\mathcal{A}=(Q,A)$ is obtained by removing the edge labels in the usual
graphical representation of $\mathcal{A}$. More formally, $D(\mathcal{A})$ has
$Q$ as the vertex set and an edge $q\to q'$ for each transition $q\cdot a=q'$.}
are Eulerian. In all these papers 1-extensibility is obtained via
linear-algebraic arguments;\marginpar{\textbf{If space\\
permits!!}} we include here a proof from~\cite{Kari:2003} as a representative
example of these linearization techniques.
...
in $D(\mathcal{A})$ has in-degree $k$ and for every subset $P\subseteq Q$, the
equality
\begin{equation}
\label{KV:eq:eulerian} \sum_{a\in A}|Pa^{-1}|=k|P|
\end{equation}
holds true since the left-hand side of \eqref{KV:eq:eulerian} is the number of
edges in $D(\mathcal{A})$ with ends in $P$. The equality \eqref{KV:eq:eulerian}
readily implies that for each $P\subseteq Q$, one of the following alternatives
takes place: either $|Pa^{-1}|=|P|$ for all letters $a\in A$ or $|Pb^{-1}|>|P|$
for some $b\in A$. Now assume that a subset $S\subseteq Q$ and a word $u\in
A^+$ are such that $|Su^{-1}|\ne|S|$ and $u$ is a word of minimum length with
this property. We write $u=aw$ for some $a\in A$ and $w\in A^*$ and let
$P=Sw^{-1}$. Then $|P|=|S|$ by the choice of $u$ and $Pa^{-1}=Su^{-1}$ whence
$|Pa^{-1}|\ne|P|$. Thus, $P$ must fall into the second of the above
alternatives and so $|Pb^{-1}|>|P|$ for some $b\in A$. The word $v=bw$ has the
same length as $u$ and has the property that $|Sv^{-1}|>|S|$. Having this in
mind, we now aim to prove that for every proper subset $S\subset Q$, there
exists a word $u\in A^*$ of length at most $n-1$ such that $|Su^{-1}|\ne|S|$.
...
form the usual orthonormal basis of $\mathbb{R}^n$. Observe that for any vector
$x\in\mathbb{R}^n$, the inner product $\langle x,[Q]\rangle$ is equal to the
sum of the coordinates of $x$ in this basis. In particular, for each subset
$P\subseteq Q$, we have $\langle[P],[Q]\rangle=|P|$. Further, assign to each
word $w\in A^*$ the linear operator $\varphi_w$ on $\mathbb{R}^n$ defined by
...
$\langle\varphi_u([S])-[S],[Q]\rangle\ne0$ . The vector $[S]$ can be uniquely
represented as $[S]=x+\lambda[Q]$ where $x\in\mathbb{R}^n$ is a vector
orthogonal to $[Q]$ and $\lambda\in\mathbb{R}$; observe that $x\ne0$ as $S\ne
Q$. Since $Qu^{-1}=Q$ for every word $u$, we have $\varphi_u([Q])=[Q]$. Hence
\begin{multline*}
\langle\varphi_u([S])-[S],[Q]\rangle=\langle\varphi_u(x+\lambda[Q])-(x+\lambda[Q]),[Q]\rangle=\\
\langle\varphi_u(x)+\lambda[Q]-x-\lambda[Q]),[Q]\rangle=\langle\varphi_u(x)-x,[Q]\rangle=
\langle\varphi_u(x),[Q]\rangle
\end{multline*}
because  $\langle x,[Q]\rangle=0$. Thus, a word $u$ satisfies $|Su^{-1}|\ne|S|$
if and only if the vector $\varphi_u(x)$ lies beyond the subspace $U$ of all
vectors orthogonal to $[Q]$.
\end{proof}

%Recent results in this direction are due to
%Steinberg~\cite{Steinberg:2010,Steinberg:2010archive}
%
%=====================================================
